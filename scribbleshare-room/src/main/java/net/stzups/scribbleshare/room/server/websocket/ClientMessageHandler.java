...
public class ClientMessageHandler extends SimpleChannelInboundHandler<ClientMessage> {
    ...
    protected void channelRead0(ChannelHandlerContext ctx, ClientMessage message) throws ClientMessageException {
        try {
            switch (state) {
                case HANDSHAKE:
                    handleHandshakeState(ctx, message);
                    break;
                case READY:
                    handleReadyState(ctx, message);
                    break;
                default:
                    throw new ClientMessageException(message, "Can't handle message while in unknown state " + state);
            }
    ...
    private void handleHandshakeState(ChannelHandlerContext ctx, ClientMessage message) throws ClientMessageException, IndexOutOfBoundsException {
        switch (message.getMessageType()) {
            ...
            default:
                throw new ClientMessageException(message, "Unsupported message for state " + state);
        }
    }

    private void handleReadyState(ChannelHandlerContext ctx, ClientMessage message) throws ClientMessageException, IndexOutOfBoundsException {
        switch (message.getMessageType()) {
            case CANVAS_UPDATE: {
